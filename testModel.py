...
import seq2seq_model  # 在翻译模型中，引入seq2seq_model
import os
import numpy as np
...
with tf.Session() as sess:  # 打开作为一次会话
    # 恢复前一次训练
    ckpt = tf.train.get_checkpoint_state('.\ckpt')  # 从检查点文件中返回一个状态(ckpt)
    # 如果ckpt存在，输出模型路径
    if ckpt != None:
    ...
    while True:
        ...
        bucket_id = min(
            [b for b in range(len(buckets)) if buckets[b][0] > len(input_string_vec)])  # 保留最小的大于输入的bucket的id
